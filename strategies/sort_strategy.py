...
from strategies.isort_strategy import ISortStrategy # Путь до твоего файла с интерфейсом
...
class SortByPriorityStrategy(ISortStrategy):
    def sort(self, tasks: List[ITask]) -> List[ITask]:
        # Маппинг приоритетов в числа для правильной сортировки (1 - наивысший)
        priority_map = {'High': 1, 'Medium': 2, 'Low': 3}
        return sorted(tasks, key=lambda task: priority_map.get(getattr(task, 'priority', 'Low'), 4))

# 3. Класс-контекст, который использует эти стратегии
class TaskSorter:
