...
with st.sidebar:
    ...
    with st.form("add_task_form", clear_on_submit=True):
        ...
        task_type = st.selectbox("Task Type", ["simple", "recurring"])
        priority = st.select_slider("Priority", options=["Low", "Medium", "High"])
        
        if st.form_submit_button("Add Task"):
            if title:
                new_task = TaskFactory.create_task(task_type, title, priority=priority)
...
st.markdown("Implemented using Spec-Driven Development (SDD) principles.")
